...
public class MemoryAccess implements Element {
		...
		if (EX_MA_Latch.isMA_enable()) {
			...
			if (!EX_MA_Latch.getMABusy()) {
				...
				if (opcode == 22) {
					...
					System.out.println("Memory Read event Registered at " + Clock.getCurrentTime()+ " for "+(Clock.getCurrentTime() + Configuration.mainMemoryLatency));
				}

				else if (opcode == 23) {
					containingProcessor.getMainMemory().setWord(EX_MA_Latch.getAluResult(), EX_MA_Latch.getOp2());
					Simulator.getEventQueue().addEvent(new MemoryWriteEvent(Clock.getCurrentTime() + Configuration.mainMemoryLatency, this, containingProcessor.getMainMemory(), EX_MA_Latch.getAluResult(), EX_MA_Latch.getOp2()));
					EX_MA_Latch.setMABusy(true);

					System.out.println("Memory Write event Registered at " + Clock.getCurrentTime()+ " for "+(Clock.getCurrentTime() + Configuration.mainMemoryLatency));
	...
	public void handleEvent(Event e) {
		...
		MA_RW_Latch.setRW_enable(true);
		System.out.println("Memory Read complete from MA at " + Clock.getCurrentTime());
	}
