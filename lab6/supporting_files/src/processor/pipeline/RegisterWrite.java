...
public class RegisterWrite {
	...
	public void performRW() {
		...
		if (MA_RW_Latch.isRW_enable()) {
			...
			if (isWb) {System.out.println("inside RW-Wb");
				int rd = 0;
				if (!isLd) {System.out.println("inside RW-notLd");
					if (opType == 3) {
					...
					System.out.println("wrote "+MA_RW_Latch.getAluResult()+" at "+rd);
				}
				if (isLd) {System.out.println("inside RW-Ld");
					rd = MA_RW_Latch.getInstruction() << 10;
					rd = rd >>> 27;
					System.out.println("rd= "+rd+" LdResult= "+ MA_RW_Latch.getLdResult());
					containingProcessor.getRegisterFile().setValue(rd, MA_RW_Latch.getLdResult());
				}
			}
			if (opCode == 29) {
				Simulator.setSimulationComplete(true);
				System.out.println("Simulation Complete: True");
			}
			...
			System.out.println("Done:"+MA_RW_Latch.getInstruction());
			MA_RW_Latch.setInstruction(0);
		}
		if((instruction >>> 27)!=29){
			IF_EnableLatch.setIF_enable(true);
