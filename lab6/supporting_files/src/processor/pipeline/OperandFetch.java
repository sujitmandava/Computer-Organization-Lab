	...
	public void performOF()
	{
		...
		if(IF_OF_Latch.isOF_enable())
		{
			if(!OF_EX_Latch.isEX_busy()){
				...
				switch(opcode)
						{
							...
							case 28:{
								...
								imm = (instruction<<15)>>15;
								immx = imm;
								branchTarget = imm + IF_OF_Latch.getInstructionLocation();
								op1 = containingProcessor.getRegisterFile().getValue(rs1);
								op2 = containingProcessor.getRegisterFile().getValue(rd);
							...
							case 24:{
								...
								branchTarget = imm + containingProcessor.getRegisterFile().getValue(rd) + IF_OF_Latch.getInstructionLocation();
							}
							case 29:
							{
								break;
				...
				// if(MA_RW_Latch.getStall()&&(!EX_MA_Latch.getMABusy()))
				// {
				// 	rdMA =-1;
				// 	rdMA2 =-1;
				...
				else if(dataConflictExist)
				{
					...
					System.out.println("1>IF DISABLED");
				...
				if ((opcode == 29)&&(!controlConflictExist)) {
					IF_EnableLatch.setIF_enable(false);
					System.out.println("2>IF DISABLED");
				}
